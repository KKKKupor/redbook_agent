...
## 2026-08-10 — prompt增强+JSON修复（计划中）
...
## 已知局限
- **JSON解析不稳定**: DeepSeek频繁返回纯文本而非JSON（`Expecting value: line 1 column 1`），触发fallback默认值。需要加`_repair_json`（同generator修复方案）。
- **默认值固化**: fallback的关键词"性格/测试/人格"几乎每次相同。
- **LLM先验知识可能过时**: DeepSeek训练数据截止时间不确定，可能与小红书当前热点脱节。

## 调优指南
- 在 `main.py` 中添加 `_repair_json` 修复JSON解析
